...
class VectorStoreAdapter:
    ...
    def upsert(self, ids: list[str], documents: list[str], embeddings: list[list[float]], metadata: list[dict]) -> None:
...
class SentenceTransformerEmbedder(EmbeddingAdapter):
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        try:
            ...
            logger.info(f"SentenceTransformerEmbedder loaded with model '{model_name}'.")
        except ImportError:
...
class ChromaVectorStore(VectorStoreAdapter):
    ...
    def __init__(self, collection_name: str = "adam_memory", path: str = str(PROJECT_ROOT / "vector_store")):
        try:
            import chromadb
            os.makedirs(path, exist_ok=True)  # Ensure directory exists
            self.client = chromadb.PersistentClient(path=path)
            self.collection = self.client.get_or_create_collection(name=collection_name)
            logger.info(f"ChromaVectorStore connected to collection '{collection_name}'.")
        except ImportError:
    ...
    def upsert(self, ids: list[str], documents: list[str], embeddings: list[list[float]], metadata: list[dict]):
